...
import { BUILTIN_ACTIONS } from "./const";
import { RunAt } from "./types";
...
function parseAutomation(tokens: Token[]): ScriptAutomation {
  ...
  return {
    pattern: urlRegStr as string,
    runAt: stage2runAt(stage as string)!,
    instructions,
    env,
  };
}
...
function parseAutomationStart(tokens: Token[], env: Env) {
  const rules: Rule[] = [
...
function parseApplyExp(tokens: Token[], env: Env): ScriptInstruction {
  ...
  return {
    action: headParams.action as string,
    args: {
      ...middleParams,
    },
    env,
...
function parseAssignExp(tokens: Token[], env: Env) {
...
function matchTokensWithRules(tokens: Token[], rules: Rule[], env: Env) {
  return tokens.reduce((memo, token, index) => {
    ...
    const part = matchRule(token, rule, env) as Record<string, EnvValueType>;
